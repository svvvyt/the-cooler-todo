...
import { List, AddList, Sidebar, Tasks } from './components/exports';
...
export default function App() {
  ...
  const onRemoveTask = (listId, taskId) => {
    if (window.confirm('Вы действительно хотите удалить задачу?')) {
      ...
      setLists(newList);
      axios.delete(`http://localhost:3001/tasks/${taskId}`).catch('Не удалось удалить задачу');
    }
  };
  ...
  const onCompleteTask = (listId, taskId, completed) => {
    ...
    axios
      ...
      .catch('Не удалось обновить статус задачи');
  };
